...
import { monthlyPlans, yearlyPlans, onetimePlans, PricingCardProps } from "@/lib/data/home-data"
...
const PricingCard = ({ title, price, description, features, actionLabel, popular, isYearly, isMonthly }: PricingCardProps) => (
  <Card
    ...
    <div>
      <CardHeader className="pb-8 pt-4">
        ...
        <div className="flex gap-0.5">
          <h3 className="text-3xl font-bold">{price==="Custom" ? "custom" : `₦${price?.toLocaleString("en-US")}`}</h3>
          <span className="flex flex-col justify-end text-sm mb-1">{isYearly ? "/year" : isMonthly ? "/month" : null}</span>
        </div>
...
export function DonateOptions() {
  ...
  const togglePricingPeriod = (value: string) => setOptions(parseInt(value) === 0 ? "monthly" : parseInt(value) === 1 ? "yearly" : "one-time")

  return (
    <div className="flex flex-col items-center justify-center w-full space-y-10">
      <PricingHeader title="Support Our Courses" subtitle="Choose a donation plan that's right for you" />
      <PricingSwitch onSwitch={togglePricingPeriod} />
      <section className="flex flex-col sm:flex-row sm:flex-wrap justify-center gap-8 mt-8">
        {options === "monthly" && monthlyPlans.map((plan) => {
          return <PricingCard key={plan.title} {...plan} />
        })}
        {options === "yearly" && yearlyPlans.map((plan) => {
          return <PricingCard key={plan.title} {...plan} />
        })}
        {options === "one-time" && onetimePlans.map((plan) => {
          return <PricingCard key={plan.title} {...plan} />
        })}
      </section>
      <br className="my-8"/>
      <Quote className="w-5/6 md:w-4/5 lg:w-3/5 border-accent mx-auto mt-8">
