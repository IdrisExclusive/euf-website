...
const buttonWithDropdownVariants = cva(
  "inline-flex items-center rounded-full font-semibold transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      variant: {
        default: "bg-primary text-zinc-100 shadow-md",
        cta: "text-zinc-100 bg-primary shadow-lg shadow-secondary",
...
export interface buttonWithDropdownProps
  extends React.HTMLAttributes<HTMLDivElement>,
    VariantProps<typeof buttonWithDropdownVariants> {}

const ButtonWithDropdown = React.forwardRef<
  HTMLDivElement,
...
>(({ className, children, ...props }, ref) => (
  <button
    className={cn(
      className,
      "flex h-full px-4 rounded-l-full justify-center items-center basis-4/5 border-r-2 border-background focus:outline-none"
    )}
    ref={ref}
    {...props}>
    {children}
  ...
  <DropdownMenu>
    <DropdownMenuTrigger asChild className="group">
      <button
        className={cn(
          className,
          "h-full px-2 rounded-r-full basis-1/5 focus:outline-none"
        )}
        ref={ref}
        {...props}>
        <ChevronDownIcon className="mx-auto transition-transform delay-75 text-zinc-100 h-[1.2rem] w-[1.2rem] group-hover:scale-110 group-hover:translate-y-[2px] group-data-[state=open]:scale-110 group-data-[state=open]:rotate-180 focus:outline-none" />
